...
async def query_ollama(prompt: str, image_data: Optional[str] = None) -> Dict[str, Any]:
    data = {
        ...
        "prompt": (
            ...
            "Estimate distances and sizes in meters (m) based on your best judgment, using common objects for reference if possible. "
            ...
            "For each command, include 'speed' (0.0 to 0.1) and 'duration' (1.0 to 5.0) in 'parameters'. "
        ...
        for _ in range(iterations):
            if jetbot_websocket:
                command_message = json.dumps({
                    "command": command,
                    "parameters": {"speed": 0.2, "duration": 1.0}  # Default parameters
                })
