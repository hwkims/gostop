...
async def text_to_speech(text: str) -> io.BytesIO:
    ...
    audio_stream = io.BytesIO()  # In-memory byte stream
...
async def generate(request_data: OllamaRequest):
    ...
    try:
        ...
        # TTS를 백그라운드 태스크로 실행하지 않고, 여기서 바로 생성
        tts_stream = await text_to_speech(full_response_text)
        tts_data = base64.b64encode(tts_stream.read()).decode("utf-8") # Base64

        return JSONResponse({
            "response": full_response_text,
            "prompt": request_data.prompt,
            "tts_data": tts_data,  # Base64 인코딩된 TTS 데이터
        })
