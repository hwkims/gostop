...
DELAY_SECONDS = 0.1
MAX_SPEED = 0.4
MAX_STEERING_INPUT = 2.0  # steering 입력 범위 확장
...
async def read_root():
    index_path = STATIC_DIR / "index_0325_2.html"
    if not index_path.exists():
        raise HTTPException(status_code=404, detail="index_0325_2.html not found")
...
async def query_ollama(prompt: str, image_data: Optional[str] = None) -> Dict[str, Any]:
    data = {
        ...
        "prompt": (
            ...
            "Generate actionable commands for the vehicle in JSON format using these commands: 'forward', 'backward', 'left', 'right', 'stop', 'u_turn'. "
            "For each command, include 'speed' (0.0 to 0.4) and 'steering' (-2.0 to 2.0, where -2 is extreme left, 0 is straight, 2 is extreme right) in 'parameters'. "
            ...
            "For U-turns ('u_turn' command), use maximum steering (-2.0 or 2.0) with low speed (0.1 or lower) to complete a 180-degree turn safely. "
            ...
            "If an obstacle appears in front of you within 30 cm, drive at 0.5 times your current speed and adjust steering to avoid it smoothly. "
            "Adjust steering in increments of 0.1 for fine control (e.g., -1.9, -1.8, ..., 1.9, 2.0). "
...
async def process_command(command: str, parameters: Dict[str, Any]):
    ...
    if command in ["forward", "backward", "left", "right", "stop", "cruise"]:
        ...
        for cmd in commands:
            ...
            cmd_params["steering"] = max(min(raw_steering / MAX_STEERING_INPUT, 1.0), -1.0)
            # U-turn 또는 큰 회전 시 속도 조정
            ...
            if driver_websocket and driver_command != "none":
                ...
                await driver_websocket.send(command_message)
                await asyncio.sleep(0.1)  # 안정성 위해 대기 시간 증가
                encoded_audio = await generate_tts(tts_text)
                await broadcast_to_clients({
                    "response": tts_text,
