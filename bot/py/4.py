...
1.  **JSON 메모리 읽기:** 'memory.json' 파일에서 최근 50개 이벤트를 읽어서 과거의 경험을 확인해.
2.  **이미지 분석:**
...
6.  **JSON 메모리 업데이트:** 현재 상황, 사용자 명령/질문, 수행한 작업, 결과, 생성한 JSON 출력, `speech_output`을 JSON 파일에 새로운 이벤트로 추가해.
...
def query_ollama(prompt: str, image_data: Optional[str] = None, context: Optional[list] = None):
    """Ollama API 호출"""
    data = {
        "model": "gemma3:4b",
        "prompt": prompt,
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Ollama API 요청 오류: {e}")
...
@app.post("/generate")
async def generate_response(request_data: OllamaRequest):
    """Ollama API 호출, 응답 생성"""
    try:
...
@app.post("/tts_stream")
async def text_to_speech_stream(request_data: dict):
    """텍스트를 음성으로 변환, 스트리밍"""
            ...
            if user_input or image_data:
                ...
                if "error" in ollama_response:
                    ...
                    try:
                        ...
                        response_text = response_json["speech_output"]
                        context = ollama_response.get('context', [])
                        await websocket.send_json({"text": response_text, "json_data": response_json})
                    except json.JSONDecodeError:
                        await websocket.send_json({"error": "Invalid JSON response from Ollama"})
            else:
