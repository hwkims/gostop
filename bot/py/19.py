# 1. gstreamer로 구동시켜보기 v
# 2. 프롬포트 : 시력이 안좋은 운전자가 운전을 원활히 할 수 있도록 함(blurred, lack clarity 등의 발언 자제)
# 3. 자율주행 모드 횟수 설정 없이, 그냥 on/off만
# 4. duration 없애기
# 5. 시속 100km/h를 모터스피드=1로 설정해서 운전
#
...
async def read_root():
    index_path = STATIC_DIR / "index_0324_3.html"
    if not index_path.exists():
        raise HTTPException(status_code=404, detail="index_0324_3.html not found")
...
async def autonomous_control(request_data: OllamaRequest):
    ...
    while is_autonomous_running:
        ...
        # 모든 명령의 tts를 하나의 문장으로 결합
        combined_tts = " ".join(cmd.get("tts", f"Executing {cmd.get('command', 'none')}.") for cmd in commands if cmd.get("command") != "none")
        if not combined_tts:
        ...
        # 한 번에 클라이언트에 전송 (JSON과 음성만)
        await broadcast_to_clients({
            "driver_command": [cmd["command"] for cmd in commands],  # 명령 리스트로 전송
            "audio": "data:audio/mp3;base64," + encoded_audio,
            "description": description
        })
